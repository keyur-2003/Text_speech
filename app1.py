...
# GOOGLE_API_KEY = "AIzaSyDZ2Nzx86oC5xXjCWKX20ThwAiYluFz5r4"
...
#         tts.save("speech.mp3")  # Save to a temporary file
#         audio = AudioSegment.from_mp3("speech.mp3")
#         os.remove("speech.mp3")  # Clean up the temporary file
#         return audio
#     except Exception as e:
...
#     return AudioSegment.silent(duration=0) # Return silent audio if no music generated

# def combine_audio(speech_audio, music_audio, music_volume_adjust=-20):
#     if isinstance(speech_audio, str):
#         return speech_audio  # Return error message if speech generation failed
#     if isinstance(music_audio, str):
#         music_audio = AudioSegment.silent(duration=len(speech_audio)) # Create silent audio of same length
#     else:
#         music_audio = music_audio + music_volume_adjust # Reduce music volume

#     combined = speech_audio.overlay(music_audio)
#     return combined.export(format="mp3").read()

# st.title("Text to Speech with (Optional) Background Music")
...
#             else:
#                 st.error(final_audio_bytes) # Display error from combine_audio
#         else:
#             st.error(speech_audio) # Display error from text_to_speech
#     else:
